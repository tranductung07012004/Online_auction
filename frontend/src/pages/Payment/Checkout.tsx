...
const Checkout: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        // Nếu đang ở môi trường dev, dùng fake data từ localStorage nếu có, hoặc tự build từ cart API
        ...
        if (orderStr) {
          try {
            ...
            if (
              ...
              orderData.photographyItems.length > 0
            ) {
              const processedPhotographyItems = orderData.photographyItems.map(
                (item: any) => ({
                  id: item.serviceId,
            ...
            if (allCartItems.length > 0) {
              ...
              setSelectedItemIds(
                new Set(allCartItems.map((_, index) => index))
              );
              setIsLoading(false);
              return;
            }
        ...
        // Nếu không có currentOrder, fallback lấy cart từ backend
        ...
        if (cartResponse.data.success && cartResponse.data.data) {
          ...
          const processedItems = cartItems.map((item: any) => ({
            dressId:
              typeof item.dress === "object"
                ? item.dress._id
                : item.dressId || item.dress,
            name: typeof item.dress === "object" ? item.dress.name : item.name,
            image:
              typeof item.dress === "object" && item.dress.images
                ? item.dress.images[0]
                : item.image,
            size:
              typeof item.size === "object" ? item.size.name : item.sizeName,
            color:
              typeof item.color === "object" ? item.color.name : item.colorName,
            quantity: item.quantity,
            pricePerDay:
              typeof item.dress === "object" && item.dress.dailyRentalPrice
          ...
          if (processedItems.length > 0) {
            ...
            let itemStartDate = firstItem.startDate
              ? new Date(firstItem.startDate)
              : today;
            let itemEndDate = firstItem.endDate
            ...
            // Auto-select all items
            setSelectedItemIds(
              new Set(processedItems.map((_, index) => index))
            );
          } else {
        ...
        // Dev fallback: nếu backend/cart API lỗi, vẫn hiển thị UI với dữ liệu giả
        if (import.meta.env.DEV) {
          console.warn(
            "Backend not available, using mock data for Checkout in dev mode"
          );
    ...
    try {
      ...
      try {
        ...
        const selectedItems = cartItems.filter((_, idx) =>
          selectedItemIds.has(idx)
        );
        if (selectedItems.length === 0) {
          throw new Error("Vui lòng chọn ít nhất một sản phẩm để thanh toán");
        }

        // Check what types of items we have in the selected items
        const selectedPhotographyItems = selectedItems.filter(
          (item) => item.isPhotographyService
        );
        const selectedDressItems = selectedItems.filter(
          (item) => !item.isPhotographyService
        );
        ...
        if (selectedPhotographyItems.length > 0) {
          ...
          // Convert to format expected by processPhotographyBookings
          const photographyItemsForBooking = selectedPhotographyItems.map(
            (item) => ({
              serviceId: item.id,
        ...
        if (hasDressItems) {
          try {
            ...
            console.error(
              "Error creating order from backend:",
              createOrderError
            );
            ...
            if (selectedDressItems.length > 0) {
              ...
              mockStartDate = firstItem.startDate
                ? new Date(firstItem.startDate)
                : new Date();
              mockEndDate = firstItem.endDate
                ? new Date(firstItem.endDate)
                : new Date();
            } else if (selectedPhotographyItems.length > 0) {
              // Use booking date for photography items if no dress items
            ...
            if (selectedPhotographyItems.length > 0) {
              ...
              const processedPhotographyItems = selectedPhotographyItems.map(
                (item) => ({
                  id: item.id,
                  name: item.name,
                  type: item.type,
            ...
            selectedItems.forEach((item) => {
              if (item.isPhotographyService) {
                ...
                if (item.purchaseType === "buy") {
                  const purchasePrice =
                    item.purchasePrice || item.pricePerDay * 10;
                  calculatedSubtotal += purchasePrice * (item.quantity || 1);
                } else {
                  calculatedSubtotal +=
                    (item.pricePerDay || 0) * days * (item.quantity || 1);
                }
              }
            });
            ...
            calculatedShipping =
              calculatedSubtotal >= 100 ? 0 : calculatedSubtotal > 0 ? 10 : 0;
            ...
            newOrder = {
              ...
              depositAmount: calculatedDeposit, // Sử dụng số tiền đặt cọc 50%
              depositPaid: true, // Đánh dấu đã thanh toán đặt cọc
              remainingPayment: calculatedRemaining, // Lưu số tiền cần thanh toán còn lại
              notes:
                "Khách hàng đã thanh toán 50% đặt cọc. 50% còn lại sẽ thanh toán khi trả váy.",
          ...
          // Use selected photography items only
          const processedPhotographyItems = selectedPhotographyItems.map(
            (item) => ({
              id: item.id,
              name: item.name,
          ...
          console.error(
            "No items found in cart (neither dresses nor photography services)"
          );
          throw new Error(
        ...
        throw new Error(
          apiError.message ||
            "Server error during order creation. Please try again later."
        );
      }
    } catch (error) {
  ...
  const processPhotographyBookings = async (
    photographyItems: any[],
    paymentMethod: PaymentMethod
  ) => {
    try {
      ...
      photographyItems.forEach((item, index) => {
        if (!item.serviceId && !item.id) {
          console.error(
            `Missing serviceId in photography item at index ${index}:`,
            item
          );
      ...
      bookingItems.forEach((item, index) => {
        if (!item.serviceId) {
          console.error(
            `Missing serviceId in transformed booking item at index ${index}:`,
            item
          );
      ...
      // Calculate totals
      const totalAmount = photographyItems.reduce(
        (sum, item) => sum + (item.price || 0),
        0
      );
      ...
      console.log(
        "Sending photography booking request to backend:",
      ...
      try {
        ...
        console.log(
          "Photography bookings created successfully:",
          response.data
        );
        return response.data;
      } catch (apiError) {
        console.error(
          "Error creating photography bookings with API:",
          apiError
        );
        console.error(
          "Error details:",
        ...
        const mockBookings = bookingItems.map((item, index) => {
          const matchingItem = photographyItems.find(
            (p) => p.serviceId === item.serviceId || p.id === item.serviceId
          );
          if (!matchingItem) {
            console.error(
              `Cannot find matching photography item for booking item at index ${index}:`,
              item
            );
            throw new Error(
              "Cannot find matching photography item for booking"
            );
          }
      ...
      <div className="container mx-auto px-4 py-8 max-w-6xl">
        ...
        <div className="bg-white rounded-lg shadow-md p-8 text-center">
          ...
          <p className="text-gray-600 mb-6">
            Your cart appears to be empty. Please add some items before
            checkout.
          </p>
          <button
    ...
    <div className="container mx-auto px-4 py-8 max-w-6xl">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2">
          ...
          {/* Bỏ phần chi tiết đơn hàng (thông tin + shipping) như yêu cầu */}
          <PaymentForm
        ...
        <div className="lg:col-span-1">
          <div className="bg-white rounded-lg shadow-md p-6 mb-6">
            ...
            <div className="border-t border-gray-200 pt-4 mt-4">
              {(() => {
                ...
                selectedShipping =
                  selectedSubtotal >= 100 ? 0 : selectedSubtotal > 0 ? 10 : 0;
                const selectedTotal =
                  selectedSubtotal + selectedTax + selectedShipping;
                  ...
                  <>
                    ...
                    <div className="flex justify-between py-2">
                      ...
                      <span className="text-sm">
                        {selectedShipping === 0
                          ? "Miễn phí"
                          : formatCurrency(selectedShipping)}
                      </span>
                    </div>
          ...
          <div className="bg-gray-50 border border-gray-200 rounded-lg p-4 space-y-3">
            <div className="bg-[#f8f3f0] p-3 rounded-lg border border-[#c3937c]">
              <p className="text-sm font-medium text-[#c3937c] mb-1">
                Lưu ý về đặt cọc
              </p>
              <p className="text-xs text-gray-700">
                Khoản thanh toán này chỉ là 50% đặt cọc của tổng giá trị đơn
                hàng. Khoản 50% còn lại sẽ được thanh toán khi bạn trả váy. Bạn
                sẽ nhận được thông báo qua email khi đến hạn thanh toán phần còn
                lại.
              </p>
            </div>
            <p className="text-xs text-gray-600">
              Bằng cách hoàn tất thanh toán, bạn đồng ý với Điều khoản dịch vụ
              và xác nhận rằng bạn đã đọc Chính sách bảo mật. Thông tin thanh
              toán của bạn được mã hóa an toàn và thông tin của bạn (ngoại trừ
              chi tiết thanh toán) sẽ được chia sẻ với OX bride.
            </p>
          </div>
        </div>
      </div>
